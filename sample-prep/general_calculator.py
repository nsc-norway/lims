...
# This script performs a calculation of sample normalisation. It can be used in 
# multiple steps.
...
for i, width in enumerate([20, 16, 8, 8, 6, 12, 12]):
...
for row_index, (input, output) in enumerate(sorted(inputs_outputs, key=sort_key), 2):
    ...
    input_ng = get_or_default('Amount of DNA (ng)')
