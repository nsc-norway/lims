...
# This is a specialised version of the script: for EKG, there is a minimum volume 1 uL.
...
def main(process_id, concentration_source, file_id, default_vol):
    ...
    for index, (input, output, sample, input_conc) in enumerate(sorted(i_o_s, key=sort_key)):
        ...
        if input_conc == 0.0:
            ...
            if sample_volume < 1:
                # Special case for EKG group: The robot can't pipette volumes less than 1 microliter,
                # so we set a lower limit. The normalised mass of DNA will be greater than the 
                # specific target as an inevitable consequence of this.
                norm_mass *= 1 / sample_volume
                sample_volume = 1
