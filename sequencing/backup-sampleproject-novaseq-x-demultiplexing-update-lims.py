...
### BACK-UP OF CHANGES USED TO ENABLE Sample_Project COLUMN ###


# This was developed in May 2025 and never (not yet) put into production.
# Sample_Project is not as useful as it would seem.
...
def get_s_numbers(analysis_dir):
    ...
    for path in paths:
        ...
        # Data/{ProjectName/}{AppName}/fastq{OraOrNot}/{ProjectName/}/{Sample_ID}_S{???}_L00{LaneID}_R{ReadNum}_001.fastq.{CompressionType}
        # The element {ProjectName/} is only included in the path if Sample_Project is used in the sample sheet.
        m = re.match(r"Data/(.*)/(ora_)?fastq/([^/]+/)?(.*)_S([0-9]+)_L00[0-8]_R[0-9]_001.fastq.(gz|ora)", path)
        if m:
            mapping[m.group(4)] = int(m.group(5))

    return mapping


def check_used_sample_project(analysis_dir):
    """We repeat the success of get_s_number and use the manifest file to determine whether sample_project is used."""
...
def get_sample_identity_matching(process, have_used_sample_project):
    ...
    for i, o in process.input_output_maps:
        ...
        # Determine if sample project name is used
        if have_used_sample_project:
            sample_info['samplesheet_sample_project'] = sample.project.name
        # Project type and delivery method are required downstream, put dummy values if no UDF
        sample_info['project_type'] = sample.project.udf.get('Project type', 'UNKNOWN')
        sample_info['delivery_method'] = sample.project.udf.get('Delivery method', 'UNKNOWN')
...
def process_analysis(run_dir, analysis_dir):
    ...
    # Path format when Sample_Project is disabled:
    if not qmetrics_paths:
        qmetrics_paths = glob.glob(os.path.join(analysis_dir, "Data",  "*",  "*fastq", "Reports", "Quality_Metrics.csv"))
    if qmetrics_paths:
    ...
    sample_id_list = get_sample_identity_matching(process, check_used_sample_project(analysis_dir))
