...
#  * Equimolar pooling only

# This script is a simplified version of "diag-pooling-worksheet-and-robot-file.py".
# It was trimmed down to only have the normal sample handling.
...
def get_sample_details(pool_input_samples, pool_pool_volume, target_sample_conc):
    ...
    target_sample_conc:   The target molarity of the pool
    """
...
def main(process_id, robot_file_name, worksheet_file_name):
    ...
    for pool in step.pools.pooled_inputs:
        ...
        # Determine concentration for normal samples / pools
        num_normal_samples = len(pool.inputs)
        target_sample_conc = pool_norm_conc * 1.0 / num_normal_samples

        # Get a list of sample information dicts
        samples, error = get_sample_details(pool.inputs, pool_pool_volume, target_sample_conc)
